...
function asyncSystemScan(mcVersion, launchAfter = true){
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    ...
                    'Pour rejoindre Oerthyon, vous devez disposer d\'une installation 64 bits de Java 8. Souhaitez-vous que nous en installions une copie ? En installant, vous acceptez <a href="http://www.oracle.com/technetwork/java/javase/terms/license/index.html" >l\'accord de licence d\'Oracle</a>.',
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            ...
                            'Une installation x64 valide de Java 8 est nécessaire pour lancer.',
                            'Je comprend',
                            'Retour'
                        )
                        setOverlayHandler(() => {
            ...
            if(m.result === true){
                ...
                setOverlayContent(
                    'Erreur inattendu:<br>Le téléchargement de Java a échouer',
                    'Malheureusement, nous avons rencontré un problème en essayant d\'installer Java. Vous devrez en installer une copie manuellement.',
                    'Je comprend'
                )
                setOverlayHandler(() => {
                    toggleOverlay(false)
    ...
    setLaunchDetails('Verification des informations systèmes..')
...
function dlAsync(login = true){
    ...
    aEx.on('close', (code, signal) => {
        if(code !== 0){
            ...
            showLaunchFailure('Erreur durant le lancement', '')
        }
    })
        ...
        if(m.context === 'validate'){
            switch(m.data){
                case 'distribution':
                    ...
                    setLaunchDetails('Chargement des informations a propos de la version..')
                    break
                case 'version':
            ...
            if(m.result.forgeData == null || m.result.versionData == null){
                ...
                loggerLaunchSuite.error('Erreur lors durant le lancement', m.result.error)
                showLaunchFailure('Erreur lors durant le lancement', '')
            ...
            if(login && allGood) {
                ...
                loggerLaunchSuite.log(`Envoie du compte (${authUser.displayName}) au ProcessBuilder.`)
                ...
                const gameStateChange = function(data){
                    ...
                    if(SERVER_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Exploring the Realm!') //TODO: Change that for discord RPC
                    } else if(GAME_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Sailing to Westeros!') //TODO: Change that for discord RPC
                    }
                }
                ...
                const gameErrorListener = function(data){
                    ...
                    if(data.indexOf('Could not find or load main class net.minecraft.launchwrapper.Launch') > -1){
                        loggerLaunchSuite.error('Erreur lors du lancement, une erreur a du ce produire durant le téléchargement.')
                        showLaunchFailure('Erreur durant le lancement', 'Le fichier principal, LaunchWrapper, n\'a pas pu être téléchargé correctement. Par conséquent, le jeu ne peut pas se lancer.<br><br>Pour résoudre ce problème, éteignez temporairement votre logiciel antivirus et relancez le jeu..')
                    }
                }
    ...
    refreshDistributionIndex(true, (data) => {
        ...
        refreshDistributionIndex(false, (data) => {
            ...
            if(DistroManager.getDistribution() == null){
                showLaunchFailure('Erreur Fatal Bazooka', 'Impossible de charger une copie de l\'index de distribution.')
