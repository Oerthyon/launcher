...
document.getElementById('logout').addEventListener('click', () => {
    AuthManager.removeAccount(ConfigManager.getSelectedAccount().uuid).then(() => {
...
function bindAuthAccountSelect(){
    Array.from(document.getElementsByClassName('settingsAuthAccountSelect')).map((val) => {
        val.onclick = (e) => {
            ...
            val.innerHTML = 'Compte selectionné &#10004;'
        ...
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                ...
                setOverlayContent(
                    ...
                    'Pour pouvoir utiliser le lanceur, vous devez être connecté à au moins un compte. Vous devrez vous reconnecter après.<br><br>Êtes-vous sûr de vouloir vous déconnecter ?',
                    'Je suis sur',
                    'Annuler'
                )
                setOverlayHandler(() => {
        ...
        authAccountStr += `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            ...
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Selectionné &#10004;' : '>Seletionnez le compte'}</button>
                    <div class="settingsAuthAccountWrapper">
                        <button class="settingsAuthAccountLogOut">D&eacute;connexion</button>
                    </div>
                </div>
            </div>
        </div>`
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    `Échec de la suppression<br>Glisser déposer de ${fullName}`,
        ...
        if(dropinUI != null){
            ...
            if(DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled){
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if(!isOverlayVisible()){
                        setOverlayContent(
                            'Échec du changement<br>Un ou plusieurs mod en glisser déposer',
                            err.message,
                            'Okay'
                        )
                        setOverlayHandler(null)
...
function populateJavaExecDetails(execPath){
    ...
    jg._validateJavaBinary(execPath).then(v => {
        if(v.valid){
            ...
            settingsJavaExecDetails.innerHTML = 'Invalide'
        }
    })
}
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/Oerthyon/Launcher/releases.atom',
        success: (data) => {
        ...
        settingsAboutChangelogText.innerHTML = 'Échec du chargement du changelog.'
    })
}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        settingsUpdateTitle.innerHTML = 'Vous lancez la dernière version'
        settingsUpdateChangelogCont.style.display = 'none'
        populateVersionInformation(remote.app.getVersion(), settingsUpdateVersionValue, settingsUpdateVersionTitle, settingsUpdateVersionCheck)
        settingsUpdateButtonStatus('Verifier les mises a jours', false, () => {
            if(!isDev){
                ipcRenderer.send('autoUpdateAction', 'checkForUpdate')
                settingsUpdateButtonStatus('Recherches des mises à jours..', true)
            }
        })
    }
}
